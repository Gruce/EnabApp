  ...
  <div>
    ...
    <div :class="{ 'd-none': selectedCustomer > 0 }" class="mt-3">
      ...
      <div class="r-2 border-0 shadow-none">
        ...
        <div class="row mt-3">
          <UtilitiesTable :data="orders" :properties="table" @watch="show = true, getProducts($event)" @customer="selectedCustomer = $event" />
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      table: {
        ...
        search: "order_number",
        control: [
          { name: "customer", icon: "fa-user", column: "customer.id" },
  ...
  methods: {
    ...
    getProducts: function (order_id) {
      this.showProducts = this.orders.find((x) => x.id == order_id).products;
    },
    ...mapActions({
